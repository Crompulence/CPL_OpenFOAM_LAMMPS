...
# Calculate the critical fluidisation velocity
...
def plot_displacement(t, xy, xySol, i, file_name='./fig'):
    # import matplotlib
    ...
    np.savez(file_name + '.npz', t=t, xy=xy, xySol=xySol, i=i)
    plt.close()
...
def test_displacement(Uf, dragModel, plot_results=True):
    ...
    # Get hydraulic gradient at end of simulation
    h, p = get_pressure_profile()
    p = p - mObj.fluid_density*mObj.gravity*np.flipud(h)
    i = -((p[0]-p[-1])/(mObj.fluid_density*mObj.gravity))/(h[0]-h[-1])
    ...
    # Plot the results
    if plot_results:
        plot_displacement(t, xy, xySol, i,
...
if __name__ == "__main__":
    test_displacement(Uf=1.85, dragModel='DiFelice', plot_results=True)
